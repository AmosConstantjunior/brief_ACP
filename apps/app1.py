...
df_total=pd.read_csv("timesData.csv")
nb_data = 50
n_comp = 8
...
modal = html.Div(
    [
        dbc.Button(style={'margin-bottom':'5vh','margin-left':'2vw','backgroundColor':'#181B1E'}, children=["Matrice des corrélations"], id="open-xl"),
        dbc.Modal(style={'backgroundColor':'#181B1E'},
            children=[
                dbc.ModalHeader("Matrice des Corrélation"),
                dbc.ModalBody(style={'backgroundColor':'#181B1E'}, children=[
            
                  dcc.Graph( 
                   figure=lay1_fig2)

                ]),
                dbc.ModalFooter(style={'backgroundColor':'#212529',},
                    children=[dbc.Button(style={'backgroundColor':'#181B1E'},children=["Fermer"], id="close-xl", className="ml-auto")
                ]),
            ],
            id="modal-xl",
            size="xl",
...
layout = html.Div(style={'backgroundColor':'#212529', 'padding-top':'2vh', 'padding-botton':'4vh'},children=[
 nav_menu,
  ...
  html.H1(style={'text-align': 'center', 'color':'#DCDCDC', 'margin-top':'2vh', 'margin-bottom':'3vh'} , children=[ "Résultats issus de l'ACP"]),
